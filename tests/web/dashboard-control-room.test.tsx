...
describe('dashboard control-room layouts', () => {
  test('admin analytics surfaces operational controls before the charts', () => {
    ...
    const ops = screen.getByText(/payout ops/i, { selector: 'p.mono-label' })
    const chart = screen.getByRole('heading', { name: /premium inflow/i })

    expect(ops.compareDocumentPosition(chart) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
  })

  test('worker dashboard shows the action center before the explanation section', () => {
    appDataMocks.useAuth.mockReturnValue({
      user: {
    ...
    const explanation = screen.getByText(/why today is priced this way/i)

    expect(actionCenter.compareDocumentPosition(explanation) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
  })
  ...
  test('worker dashboard presents the live rail before the action center', () => {
    ...
    const actionCenter = screen.getByText(/do something useful now/i)
    const pricingExplanation = screen.getByText(/why today is priced this way/i)

    expect(liveRail.compareDocumentPosition(actionCenter) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
    expect(actionCenter.compareDocumentPosition(pricingExplanation) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
  })
